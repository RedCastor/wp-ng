 ...
 *
 *
 * This template can be overridden by copying it to yourtheme/wp-ng/galleries/slick/slick-item.php.
 *
 * @package    Wp_Ng
...
<<?php echo $slickItem; ?>>
  <figure class="<?php echo implode(' ', $class_item); ?> <?php echo (!empty($item['class']) ? $item['class'] : ''); ?>" style="<?php echo implode(' ', $style) ?>" >
    ...
    <?php if ( !empty($gallery['options']['lazyLoad']) && $gallery['options']['lazyLoad'] === 'ondemand' ) : ?>
      <img class="slick-lazy <?php echo implode(' ', $class_img); ?>" data-lazy="<?php echo $item['image']; ?>" alt="<?php echo $item['alt'] ; ?>" >
    <?php else : ?>
    ...
    <?php if (!empty($item['content'])) : ?>
      <section class="slick-slide-content <?php echo (!empty($item['content_class']) ? $item['content_class'] : ''); ?>">
